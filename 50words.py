...
#             while True:
#                 # read byte by byte to avoid using RAM
#                 character = file.read(1).lower()
...
#     try:
#         res = sorted([pair for pair in sorted([x for x in frequencies.items(
#         )], key=lambda x: x[0])], key=lambda x: x[1], reverse=True)
#         for k, v in res:
#             print(f'{k} = {v}')
